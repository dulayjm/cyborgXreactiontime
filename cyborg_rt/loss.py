...
class ReactionTime(nn.Module):
    ...
    def forward(self, output: Tensor, target: Tensor, reaction_times: Tensor) -> Tensor:
        ...
        for i in range(len(preds)):
            # if preds[i] != target[i]:
            # now we're just applying it to everything
            apply_penalty = np.random.binomial(1,0.5)
            if apply_penalty:
                ce[i] += psych_penalties[i].clone()
        # then reduce to common value (mean is default)
        # https://pytorch.org/docs/stable/generated/torch.nn.CrossEntropyLoss.html
...
class MyDenseLayer(nn.Module):
    ...
    def __init__(self, c_in=3, bn_size=2, growth_rate=1):
        """
        Inputs:
...
class DenseBlock(nn.Module):
    ...
    def __init__(self, c_in, num_layers, bn_size, growth_rate, act_fn):
        ...
        for layer_idx in range(num_layers):
            layers.append(
                MyDenseLayer(c_in=c_in + layer_idx * growth_rate, # Input channels are original plus the feature maps from previous layers
                           bn_size=bn_size,
...
class DifferentiablePsychLoss(nn.Module):
    """
    Justin - 01/23/2023

    This will be the first fully differentiable version
    """
    ...
    def __init__(self):
        ...
        # using only three difficulty labels for now classes for now
        # self.fc = nn.Linear(224, 3)
        self.backbone.classifier = nn.Linear(224, 3)
    ...
    def forward(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        out = self.backbone(x)
        # so we go through all of this, it's just that this loss
        # doesn't match upt with it 


        # print('shape of out is', out.shape)
        # print('rt shape is', y.shape)
        # print('out is', out)
        ...
        # you need to do something else with the loss
        # to make it a loss here, and not just a model output ...
